is_GNN=False
# dataset='ml-100k'
# dataset='ali-ads'
dataset='ml-1m'
# pre_name='APDF-pretrain-ml.pkl'
# pre_name='apdf-pre2-titan.pkl'
pre_name='APDF-FedNCF-pretrain13-ml1m.pkl+lr0.5+eta80'
ratio=1.
is_esti_local=True

# python run_experiment.py --clients_sample_ratio $ratio --GNN $is_GNN --num_round 40 --NAME $pre_name --dataset $dataset --earlystop 15 --pretrain False --finetune True

python run_grid_search.py --GNN $is_GNN --num_round 100 --NAME $pre_name --dataset $dataset --earlystop 20 --pretrain False --finetune True --is_esti_local $is_esti_local