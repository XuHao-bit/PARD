...
class Trainer(object):
    ...
    def __init__(self, config):
        ...
        if self.config['dataset'] == 'ml-100k' or config['dataset'] == 'ali-ads':
            ...
            self.privacy_keys = ['location']
            self.privacy_ratios = [1.] # meta-param for adaptive learning
        elif self.config['dataset'] == 'bookcrossing':
            self.privacy_keys = ['age', 'location']
            self.privacy_ratios = [1., 1.] # meta-param for adaptive learning
    ...
    def get_full_input_label(self, esti_input, labels, attr, is_pu=False):
        if is_pu:
        ...
        if len(full_input) > 0:
            ...
            idx = np.random.choice(full_input.shape[0], int(full_input.shape[0]/2), replace=False)
            # idx = np.random.choice(full_input.shape[0], int(full_input.shape[0]/3), replace=False)
            if idx.shape[0] > 0:
    ...
    def fed_train_estimator(self, model_client, optimizer_esti, esti_input, privacy_labels, is_pu=False):
        for ratio, attr in zip(self.privacy_ratios, self.privacy_keys):
            ...
            for epoch in range(self.config['pries_epoch']):
                ...
                # 选取batch个，别太多了
                esti_input, labels = self.get_full_input_label(esti_input, labels, attr, is_pu)
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers, privacy_labels, pos_items):
        ...
        optimizer, optimizer_esti, optimizer_pri = optimizers
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        for uidx, user in enumerate(participants):
            ...
            if self.config['dataset'] == 'ml-100k' or self.config['dataset'] == 'ali-ads':
            ...
            for epoch in range(self.config['local_epoch']):
                for batch_id, batch in enumerate(user_dataloader):
                    ...
                    model_client, loss_batch = self.fed_train_single_batch(model_client, batch, optimizers, privacy_labels, pos_items)
    ...
    def run_experiment(self, config, sample_generator):
        ...
        if config['finetune']:
            ...
            # self.ori_server_keys = self.server_keys
            # self.ori_client_keys = self.client_keys
            # if config['dataset'] == 'ml-100k':
            #     self.server_keys = ['embedding_item.weight'] 
            #     self.client_keys = ['embedding_puser.weight', 'embedding_euser.weight'] + self.mlp_keys + self.priemb_keys # param saved in client
            # elif config['dataset'] == 'douban':
            #     self.server_keys = ['embedding_item.weight', 'embedding_euser.weight'] 
            ...
            # self.server_keys = self.ori_server_keys
            # self.client_keys = self.ori_client_keys
